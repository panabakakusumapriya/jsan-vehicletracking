      ...
      <div className="page-head">
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: 'var(--muted)', fontSize: 13 }}>
            Every manager, team lead and driver belongs to one of these
          </p>
        </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <table>
          ...
          <tbody>
            ...
            {projects.length === 0 && (
              <tr><td colSpan={5} style={{ textAlign: 'center', padding: '40px 24px', color: 'var(--muted)' }}>No projects yet — add one to get started.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function ProjectForm({ project, onClose, onSaved }: { project?: Project; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    try {
      const body = { name: form.name.trim(), code: form.code || null, country: form.country || null, active: form.active };
