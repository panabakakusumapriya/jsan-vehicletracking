...
**Addendum 2 (GPS-noise pass, 2026-09-07):** Ali Azhar's first live session exposed a noise class: 48 m avg accuracy fixes crossing the 10 m record gate minted 9.7 phantom km + a 211 km/h teleport inside a 3 km box. Fixed in TrackingService.kt (Sev 5 × Lik 4 = 20 · P0): RECORD_MAX_ACCURACY_M 35 m for minting points AND arming the trip-start watch; STILL-detector drift suppression (skip record when still && <8 km/h); teleport rejection (implied jump >180 km/h dropped). Plus background enforcement (Sev 4 × Lik 3 = 12 · P1): ACTIVITY_RECOGNITION is now a hard permission requirement (it is the only legal background restart after the 10-min idle stop), and the battery-optimisation exemption dialog is forced at startup (new module fns isIgnoringBatteryOptimizations / requestIgnoreBatteryOptimizations).

---

**Addendum 3 (bootstrap regression + hard gate, 2026-09-07):** ROOT CAUSE of 'tracking stopped when we disabled modules' — VehicleTracker.configure/start lived ONLY in home.tsx, and the project-wise tab feature routes drivers straight to /map when dashboard is disabled, so home never mounted and tracking never started (Sev 5 × Lik 5 = 25 · P0, fixed: TrackingBootstrap mounted at session level in _layout — also closes D4). Companion hardening: the pre-login permission gate is now a HARD six-for-six gate (fine+background location, activity recognition, notifications, battery exemption, GPS switch — MyCarTracks-grade; its decompiled manifest confirmed we already exceed its permission set), with battery dialog chained into Grant-All and a settings-return re-audit; a TrackingChecklist card on the dashboard surfaces post-login regressions with per-OEM tips (Xiaomi/Oppo/Vivo/Huawei/Samsung autostart & sleep lists).

**Guard addendum:** mid-session permission loss now BLOCKS (TrackingGuard overlay, 60 s + focus re-audit) — closes the launch-gate-only gap.

---

**Addendum 4 (final tracking deep-review, 2026-09-07):** adversarial pass over the whole chain. FOUND & FIXED: (P1, 15) the flat 35 m record-accuracy gate from Addendum 2 would starve pocket/bag-carried phones (35-80 m accuracy is normal in cars) — no points → no-move timer kills live trips mid-drive; replaced with ADAPTIVE gate: movement counts once it beats the fix's own error radius (minMove = max(10 m, accuracy)); trip-start stays strict (50 m cap + distance ≥ max(30 m, 1.5×accuracy)). (P4, 4) poor-accuracy idle fixes skipped the idle-timeout check — service could run forever in a bad-GPS garage; timeout now inlined on that path. (P3) Grant-All now chains the system GPS-enable dialog (enableNetworkProviderAsync). (P3) server clamps trip maxSpeed at 250 km/h against old-build teleports. VERIFIED CLEAN: no duplicate GPS callback registration (applyCadence removes-then-adds, cadence=null forces re-arm on restart), heartbeats run while idle, uploader backoff capped, boot/AR/connectivity receivers sound, sign-out stops service, launch gate + mid-session guard + checklist cover permissions at every stage. KNOWN-DEFERRED: SQLite queue unbounded (weeks of dead token = big DB; P3), wake lock held for service lifetime (P5, internal app).

---

**Addendum 5 (live road coverage, 2026-09-07):** field report 'red roads not turning blue while driving, no line drawing' — root causes: (1) tested binary predates every fix (nothing shipped yet: no trail, no dot fixes); (2) DESIGN GAP: roads only recoloured after trip end + server link attribution — zero live street-colour feedback (Sev 4 × Lik 5 = 20 · P0 as driver-awareness). FIXED: client-side live coverage (src/lib/liveCover.ts) — uncovered links grid-indexed per roads version (~220 m cells), every active fix point-to-segment matched ≤25 m, hit links flip blue immediately (MapNative merges live ids into the covered bucket; legend counts follow); set persists per roads-version for 24 h so app restarts mid-shift stay blue; a new server roads version (post-trip attribution) replaces the live guess with audited truth. Server remains the authority for UKM/pay.
